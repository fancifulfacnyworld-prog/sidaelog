...
export function CasesView({ lang }: { lang: Lang }) {
  ...
  const artHref = (slug: string) =>
    `${lang === "en" ? "/en" : ""}/articles/${slug}`;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fdfcfa] md:h-screen md:flex-row md:overflow-hidden">
                    ...
                    <div className="mt-auto flex items-center gap-3 pt-4 text-[10px] uppercase tracking-[0.08em] text-[#16140f]/40">
                      ...
                      {subCount(c.slug) > 0 && (
                        <>
                          ...
                          <span>{dict.cases.cont(subCount(c.slug))}</span>
                        </>
                      )}
                    </div>
                  </div>
      ...
      <MobileBottomNav active="cases" lang={lang} />
    </div>
  );
}
