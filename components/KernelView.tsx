...
export function KernelView({ lang }: { lang: Lang }) {
  const dict = getDict(lang);
  const kernelArticles = getAllKernelArticles(lang);
  const kHref = (slug: string) =>
    `${lang === "en" ? "/en" : ""}/kernel/${slug}`;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fdfcfa] md:h-screen md:flex-row md:overflow-hidden">
      ...
      <MobileBottomNav active="kernel" lang={lang} />
    </div>
  );
}
