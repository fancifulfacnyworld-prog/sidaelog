...
export default function Home() {
  ...
  const groups: CaseGroup[] = roots.map((root, i) => {
    const subs = articles
      ...
      .sort(byDateAsc);
    const allTags = uniqNorm([
      ...root.tags,
      ...subs.flatMap((s) => s.tags),
    ]);
  ...
  const tagGroups: TagGroup[] = [
    {
      label: "설계",
      en: "Lens",
      tags: ["감정 설계", "몰입 설계", "선택 설계", "책임 설계", "관찰 설계"],
    },
    {
      label: "관점",
      en: "Approach",
      tags: ["게임분석", "디렉터의시선", "UI설계", "UX전략"],
    },
  ];

  return <HomeShelf groups={groups} solos={soloItems} tagGroups={tagGroups} />;
}
